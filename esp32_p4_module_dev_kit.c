...
#include "bsp/esp32_p4_module_dev_kit.h"
...
// Adapt to the 2-inch circular screen and 2.1-inch circular TFT display screen, a high-definition IPS screen with RGB MIPI interface (480x480 resolution) from Shenzhen Huadichuangxian Technology Co., Ltd., a Taobao merchant.
static const st7701_lcd_init_cmd_t lcd_init_cmds[] = {
    ...
    {0xB0, (uint8_t[]){0x00, 0x1D, 0x29, 0x12, 0x17, 0x0B, 0x18, 0x09, 0x08, 0x2A, 0x06, 0x14, 0x11, 0x19, 0x1C, 0x18}, 16, 0},
    {0xB1, (uint8_t[]){0x00, 0x1D, 0x29, 0x12, 0x16, 0x0A, 0x18, 0x08, 0x09, 0x2A, 0x07, 0x13, 0x12, 0x19, 0x1D, 0x18}, 16, 0},
    {0xFF, (uint8_t[]){0x77, 0x01, 0x00, 0x00, 0x11}, 5, 0},
    {0xB0, (uint8_t[]){0x5D}, 1, 0},
    {0xB1, (uint8_t[]){0x2F}, 1, 0},
...
// The same as above.
#define DSI_PANEL_DPI_2_1_INCH_CONFIG(px_format)     \
    {                                                \
        .dpi_clk_src = MIPI_DSI_DPI_CLK_SRC_DEFAULT, \
        .dpi_clock_freq_mhz = 19,                    \
        .virtual_channel = 0,                        \
...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    uint8_t target_value = (brightness_percent * max_steps) / 100;

    if (current_brightness == target_value)
    {
        return ESP_OK;
    }
    ...
    if (current_brightness == 0)
    {
        ...
        current_brightness = max_steps;
        esp_rom_delay_us(30);
    }

    int from = max_steps - current_brightness;
    int to = max_steps - target_value;
    int num = (max_steps + to - from) % max_steps;
...
#elif CONFIG_BSP_LCD_TYPE_480_480_2_1_INCH
    esp_lcd_dpi_panel_config_t dpi_config = DSI_PANEL_DPI_2_1_INCH_CONFIG(LCD_COLOR_PIXEL_FORMAT_RGB565);
#endif
#endif
    dpi_config.num_fbs = CONFIG_BSP_LCD_DPI_BUFFER_NUMS;
